...
void *t_function(void *data) {
	...
	// 소켓 바로 종료setsockopt(server_socket, SOL_SOCKET, SO_REUSEADDR, (char *)&bf, (int)sizeof(bf));	
	if (-1 == server_socket)
	{
	...
	char admin_pw[20]="";
		...
		in_size=strlen(ans);
		out_size=sizeof(buff_snd);
		char *in_ptr = ans;
		char *out_ptr= buff_snd;
		iconv(it,&in_ptr,&in_size,&out_ptr,&out_size);
		iconv_close(it);
		write(client_socket, buff_snd, strlen(buff_snd) + 1);
		// +1: NULL까지 포함해서 전송
		printf("%s\n",buff_snd);
		close(client_socket);
		printf("	클 라 이 언 트 접 속 종 료   		\n");
		//==================================//
